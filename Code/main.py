...
from model_D import Model, EarlyStopper, shuffle_label_data
...
def main():
    ...
    # parser.add_argument("-e", "--hidden", default=18, help="specify dimension of embedding", type=int)
    ...
    # lazy init, need to rune one mode step for inferring number of parameters
    with torch.no_grad():
    ...
    for epoch in range(1_000):
        ...
        if early_stopper.early_stop(train_loss, val_loss) and epoch > 700:
            print('early stopped at epoch ', epoch)
            break
    b_t_auc = max(results_auc[2])
    b_t_aupr = max(results_aupr[2])
    end_time = time.time()
    ...
    with open(os.path.join(OUTPUT_PATH, 'results.txt'), 'a') as f:
        f.write(f'{b_t_auc:.4f}\t{b_t_aupr:.4f}\t{(end_time - init_time) / 60:.4f}\t{epoch}\n')
